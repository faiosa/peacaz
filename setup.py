...
PLUGINS = [
    path
    for path in os.listdir("urh/plugins")
    if os.path.isdir(os.path.join("urh/plugins", path))
]
URH_DIR = "urh"
...
def get_packages():
    ...
    for dirpath, dirnames, filenames in os.walk(os.path.join("./", URH_DIR)):
        package_path = os.path.relpath(
            dirpath, os.path.join("./", URH_DIR)
        ).replace(separator, ".")
...
def get_extensions():
    filenames = [
        ...
        for f in os.listdir("urh/cythonext")
        if f.endswith(".pyx")
    ]
    extensions = [
        Extension(
...
setup(
    ...
    author_email="oleh.d.myhal@gmail.com",
    package_dir={"": "."},
